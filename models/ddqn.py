...
class DQN(object):
    def __init__(self, cfg):
        ...
        self.experience_update_frequency = cfg["experience_update_frequency"]
    ...
    def _make_model(self, cfg: Dict[str, int]) -> keras.Model:
        kernel_init = HeUniform(seed=RANDOM_SEED)
    ...
    def choose_action(self, state) -> int:
        if np.random.rand() < self.epsilon:
            ...
            q_values = self.model(
                tf.convert_to_tensor(np.reshape(state, [1, 2]))
            ).numpy()
            action = np.argmax(q_values)
        return action

    def get_gap_reward(self, state, reward):
        x_pos = state[0]
        y_dif = state[1]
    ...
    def learn(self):
        ...
        q_values = self.model(tf.convert_to_tensor(states)).numpy()
        q_target_next_values = self.target_model(tf.convert_to_tensor(next_states)).numpy()
    ...
    def train(self, env: gym.Env, num_episodes: int = 20000, save_interval=250):
        ...
        for episode in range(num_episodes):
            ...
            while not terminal:
                ...
                if self.experience.index == 5000:
                    self.start_update = True
                if self.start_update and self.step_count % self.experience_update_frequency == 0:
            ...
            gc.collect()  # housekeeping
            tf.keras.backend.clear_session()  # housekeeping
            ...
            print(
                ...
                f"Total Rewards: {training_rewards:<9} "
