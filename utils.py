...
def plot_combined_scores(combined_scores: list, legend: str):
    ...
    plt.ylabel('Score')
    plt.legend()
    plt.title(f'Scores per Episode for {legend}')
    plt.grid()
...
def cumsum_average(data):
    cumsum = np.cumsum(data)
    return cumsum / np.arange(1, len(data) + 1)


def plot_cumsum_averages(data_dict):
    plt.figure()

    for legend, lst in data_dict.items():
        avg = cumsum_average(np.array(lst))
        plt.plot(avg, label=legend)

    plt.legend()
    plt.xlabel('Episodes')
    plt.ylabel('Average Score')
    plt.title('Training Average Score for Flappy Bird Models')
